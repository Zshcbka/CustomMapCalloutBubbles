...
- (id) initWithAnnotation:(id <MKAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier {
	if ((self = [super initWithAnnotation:annotation reuseIdentifier:reuseIdentifier])) {
        ...
        if([[NSBundle mainBundle] pathForResource:reuseIdentifier ofType:@"nib"] != nil)
        {
            [[NSBundle mainBundle] loadNibNamed:reuseIdentifier owner:self options:nil];
        }
        _offsetFromParent = CGPointMake(8, -14);
...
- (void)prepareOffset {
    ...
    CGFloat xOffset = _offsetFromParent.x / 2 - 7;
	...
	CGFloat yOffset = -(self.frame.size.height / 2 +
						_offsetFromParent.y / 2) - CalloutMapAnnotationViewHeightAboveParent;
...
- (void)adjustMapRegionIfNeeded {
	...
	if (pixelsFromTopOfMapView < 10)  {
		yPixelShift = 10 - pixelsFromTopOfMapView;
	} else if (pixelsFromBottomOfMapView < 10) {
		yPixelShift = -(10 - pixelsFromBottomOfMapView);
	}
    
    CGFloat pixelsFromLeft = -(mapViewOriginRelativeToParent.x - CalloutMapAnnotationViewInset - 16);
    CGFloat pixelsFromRight = self.mapView.frame.size.width + mapViewOriginRelativeToParent.x - 32 - CalloutMapAnnotationViewInset;
    NSLog(@"pixels from left and right %f, %f",pixelsFromLeft, pixelsFromRight);
    if (pixelsFromLeft < self.frame.size.width/2) {
		xPixelShift = self.frame.size.width/2 - pixelsFromLeft;
	} else if (pixelsFromRight < self.frame.size.width/2) {
		xPixelShift = -(self.frame.size.width/2 - pixelsFromRight);
	}
...
- (void)drawRect:(CGRect)rect {
	...
	color = [[UIColor grayColor] colorWithAlphaComponent:.7];
	[color setFill];
	...
	glossRect.size.height = (rect.size.height - stroke) - 5;
